...
def main():
    ...
    if version not in LINUXMINT_VERSIONS_TO_NAME:
        print(f'Unknown version: {version}')
        sys.exit(1)

    new_name = LINUXMINT_VERSIONS_TO_NAME[version]
    current_version = get_current_number()
    current_name = get_current_name()

    sorted_versions = sorted(LINUXMINT_VERSIONS_TO_NAME.keys(), key=lambda x: list(map(int, x.split('.'))))
